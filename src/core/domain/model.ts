...
export interface ContentBundle { cities: CityDefinition[]; routes: RoadDefinition[]; cargoTypes: CargoDefinition[]; vehicleModels: VehicleModelDefinition[]; orderTemplates: OrderTemplate[]; }
...
export interface GameState { saveVersion: 1; gameVersion: string; createdAt: string; updatedAt: string; contentPacks: Array<{ id: string; version: string }>; featureFlags: Record<string, boolean>; clock: { now: GameSeconds; speed: number; paused: boolean }; company: CompanyState; vehicleUnits: VehicleUnitState[]; orders: MarketOrder[]; activeRoutes: EntityId[]; eventLog: GameEvent[]; randomSeed: number; nextOrderSerial: number; migrationHistory: string[]; }
export type EngineCommand = { type: "AcceptOrder"; orderId: EntityId } | { type: "AssignTransportUnit"; orderId: EntityId; transportUnitId: EntityId } | { type: "StartTrip"; transportUnitId: EntityId } | { type: "AdvanceTime"; elapsedGameSeconds: GameSeconds };
